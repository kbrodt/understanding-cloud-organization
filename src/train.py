...
from src.dataset import CloudsDS, train_transform, dev_transform, collate_fn
...
def parse_args():
    ...
    parser.add_argument('--attention-type', type=str, default='scse')
    ...
    parser.add_argument('--batch-size', '-b', type=int, default=12,
                        help='Batch size per process (default: 12)')
...
def main():
    ...
    if args.opt == 'adam':
        opt = apex.optimizers.FusedAdam(add_weight_decay(model, args.weight_decay, ('bn', )),  # model.parameters(),
                                        lr=args.lr,
    ...
    if args.cls:
        def BCEBCE(logits, target):
            ...
            return sm.utils.losses.BCEDiceLoss(eps=1)(prediction_seg, target) + nn.BCEWithLogitsLoss()(prediction_cls, y_cls)
        ...
        BCEBCE = sm.utils.losses.BCEDiceLoss(eps=1)
    ...
    sheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt,
                                                          T_max=5,
                                                          eta_min=args.lr*(1e-3 if args.resume else 1e-2),
    ...
    train_ds = CloudsDS(train_gps, root=path_to_data / 'train', transform=train_transform)
    dev_ds = CloudsDS(dev_gps, root=path_to_data / 'train', transform=dev_transform)
